<?php
include "i/shead.php";
include "functions/validatemail.php";



if($_POST){
	if($_POST['capthca'] == $_SESSION['rand_code'] && !$_POST['validate'] && !$_POST['set_new_pwd']){

		if(check_email($_POST['username'], TRUE) == TRUE){
			$sql = "INSERT INTO `users` ( 
					`username` , 
					`password` , 
					`active` , 
					`name`,
					`lastlogin` , 
					`lastloginfrom` ,
					`createddate`
					)
					VALUES (
					'".mysql_real_escape_string($_POST['username'])."', 
					'".md5($_POST['password'])."', 
					'no', 
					'".mysql_real_escape_string($_POST['name'])."',
					'".date('Y-m-d H:i:s')."' , 
					'".$_SERVER['REMOTE_ADDR']."',
					'".date('Y-m-d H:i:s')."'
					)";
			if(mysql_query($sql)){	
					
					
					
				// to
				$to  = $_POST[username]; 
				
				// subject
				$subject = 'Godkänn skapandet av ditt konto på Skrivihop.net';
				
				// message
				$message = '
				<html>
				<head>
				  <title>'.$subject.'</title>
				</head>
				<body style="background-color: #efefec; margin:0; padding: 0;">
				  <div style="width: 100%;	height: 40px;	background-color: #687a79;	text-align: center;"></div>
				  <div style="margin-left: 5%;">
					<p>
					<img src="data:image/png;base64," alt="">
					</p>
					<h1 style="color: #3d4746;	font-weight: 100;	margin: 0 auto;">Välkommen till Skrivihop.net '.$_POST['name'].'</h1>
					<p>
					Ditt konto är skapat men du måste aktivera det också, eftersom vi vill veta att det här är din faktiska e-post adress, och att det faktiskt var du som skapade kontot, och att det var med vilje.
					</p>
					<p>
					Klicka på länken här nedanför, där kan du läsa igenom de regler som gäller för Skrivihop.net och ditt konto, godkänn dem och sen är det bara att tuta och köra, går det inte att klicka på den testa att kopiera adressen och klistra in i din webbläsare.
					</p>
					<p>
					<a href="http://skrivihop.net/templates/signup.php?validate='.base64_encode($_POST['username'].':#:'.md5(md5($_POST['password']))).'">http://skrivihop.net/templates/signup.php?validate='.base64_encode($_POST['username'].':::'.md5(md5($_POST['password']))).'</a>
					</p>
					<p>
					Ett stort tack för att du valt att skapa ett konto på <a href="http://skrivihop.net">Skrivihop.net</a>, vi ser fram emot dina texter!
					</p>
					<p>
					Om det av någon anledning inte var du som skapade det här kontot, eller något annat är fel, hör av dig till ed@skrivihop.net.
					</p>
					</div>
					<div style="width: 100% height: 40px; background-color: #687a79;	text-align: center;">
					<p style="color: #fff; font-size: 10px;">
					Det här mailet skickades till '.$_POST['username'].' från Skrivihop.net, mailet är autogenererat och en del av användarregisterings processen.
					</p>
					</div>
				</html>
				';
				
				// To send HTML mail, the Content-type header must be set
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				$headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
				
				// Additional headers
				$headers .= 'To: '.$_POST['name'].' <'.$_POST['username'].'>' . "\r\n";
				$headers .= 'From: Skrivihop.net <ed@skrivihop.net>' . "\r\n";
				
				
				// Mail it
				mail($to, $subject, $message, $headers);
		include "i/head.php";
		include "i/header.php";
		echo '<img src="images/logosmallbeta.png" class="toplogo"> 
				<br style="clear: both;">
				<h1>Vi har skickat mail till:'.$_POST['username'].'</h1>
				<p>Tack för att du registrerat din användare på Skrivihop.net, vi har skickat ett mail till din adress med en länk som du måste klicka på för att aktivera ditt konto.</p>
				<p><br></p><p><br></p><p><br></p>';
				
		include "i/footer.php";
		include "i/foot.php";
		
				}
			}
			else{
				$errormsg_arr[] = 'Ditt användarnamn är inte en giltig e-post adress, det måste det vara.';
				$errorflag = true;	
			}
	}
	elseif($_POST['yesitisokbyme'] AND $_POST['submit'] AND $_POST['validate']){
	$string = base64_decode($_POST['validate']);
	$user = explode(':#:',$string);

	
	$sql = "SELECT id FROM users WHERE username = '".mysql_real_escape_string($user[0])."' AND MD5(password) = '".mysql_real_escape_string($user[1])."'";
	$res = mysql_query($sql);
	$num = mysql_num_rows($res);
	$rad = mysql_fetch_assoc($res);
	
		if($num > 0){
			$sql_activate = "UPDATE `users` SET `active` = 'yes',
							`lastlogin` = '".date('Y-m-d H:i:s')."' 
							WHERE 
							`users`.`id` = '".$rad['id']."' 
							LIMIT 1 ;";
			if(mysql_query($sql_activate)){
				$errormsg_arr[] = 'Ditt konto är aktiverat! Nu kan du logga in.';
				$errorflag = true;
				unset($_POST);
				unset($create_new_user);
			}
			else{
				$errormsg_arr[] = 'Det blev något fel när vi försökte aktivera ditt konto, vänligen försök igen.';
				$errorflag = true;	
			}
		}
	}
	elseif(isset($_POST['set_new_pwd']) && $_POST['capthca'] == $_SESSION['rand_code']){

			$string = base64_decode($_POST['set_new_pwd']);
			$user = explode('#:#',$string);

	
	$sql = "SELECT id FROM users WHERE username = '".mysql_real_escape_string($user[0])."' AND active = 'no'";
	$res = mysql_query($sql);
	$num = mysql_num_rows($res);
	$rad = mysql_fetch_assoc($res);
	
		if($num > 0){
			$sql_activate = "UPDATE `users` SET `password` = '".md5($_POST['pass1'])."',
							`lastlogin` = '".date('Y-m-d H:i:s')."' , `active` = 'yes'
							WHERE 
							`users`.`id` = '".$rad['id']."' 
							LIMIT 1 ;";
			if(mysql_query($sql_activate)){
				$errormsg_arr[] = 'Ditt konto är aktiverat och ditt lösenord bytt! Nu kan du logga in.';
				$errorflag = true;
				unset($_POST);
				unset($create_new_user);
			}
			else{
				$errormsg_arr[] = 'Det blev något fel när vi försökte aktivera ditt konto, vänligen försök igen.';
				$errorflag = true;	
			}
		}
	}
	else{
		$errormsg_arr[] = 'Du skrev inte in rätt bokstäver, försök igen.';
		$errorflag = true;
		$create_new_user['username'] = $_POST['username'];
		$create_new_user['password'] = $_POST['password'];
		$create_new_user['name'] = $_POST['name'];
	}
}
elseif($_GET['validate']){
	

	include "i/head.php";
	include "i/header.php";
	echo '<img src="images/logosmallbeta.png" class="toplogo"> 
			<br style="clear: both;">';
	$sql_terms = "SELECT * FROM sitetexts WHERE name = 'termsandconditions'";
	$res_terms = mysql_query($sql_terms);
	$terms = mysql_fetch_assoc($res_terms);

	echo "<h1>".$terms['heading']."</h1>";
	echo "<p>".$terms['text']."</p>";
	echo "
	<form class='box login' method='post'>
			<div class='arrowup'>

			<!-- -->

			</div>
			<label>Jag accepterar villkoren för tjänsten.</label>
			<input type='hidden' name='validate' value='".$_GET['validate']."'>
			<input type='checkbox' name='yesitisokbyme' onclick='$(\"#okgo\").removeAttr(\"disabled\");'><br>";
	echo "<input type='submit' name='submit' id='okgo' value='Aktivera mitt konto' disabled='disabled'><br style='clear: both;'></form>";
	include "i/footer.php";
	include "i/foot.php";

	
}
elseif($_GET['newpassword']){
	

	include "i/head.php";
	include "i/header.php";
	include "functions/captcha.php";
	echo '<img src="images/logosmallbeta.png" class="toplogo"> 
			<br style="clear: both;">';


	echo "<h1>Sätt nytt lösenord.</h1>";
	echo "<p>Eftersom du följde länken så vet du vad du skall göra. Men för säkerhets skull så gör såhär:<br>
	Skriv in ditt nya lösenord och texten i bilden och klicka på \"Sätt nytt lösenord\".</p>";
	create_image();
	echo "
	<form class='box login' method='post'>
			<div class='arrowup'>

			<!-- -->

			</div>
			<label>Nytt lösenord.</label>
			<input type='hidden' name='set_new_pwd' value='".$_GET['newpassword']."'>
			<input type='password' name='pass1' id='pass1' placeholder='Nytt lösenord'>
			<br>
			<label for='captcha'>Skriv in texten i bilden</label>
			<input type='text' name='capthca' placeholder='Texten i bilden'><img src='capthca.png?date=".date('His')."' style='position: relative; top: 13px;'><br>
			<input type='submit' name='npwd_submit' value='Sätt nytt lösenord'/><br style='clear: both;'>
			</form><p><br></p>";
	include "i/footer.php";
	include "i/foot.php";

	
}
else{
		$errormsg_arr[] = 'Sluta göra så.';
		$errorflag = true;
			//If there are input validations, redirect back to the login form

}



if($errorflag) {
		 
		include "index.php";

			// Finally, destroy the session and exit.
			session_destroy();
			exit();
}


?>